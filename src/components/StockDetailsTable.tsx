...
export function StockDetailsTable({
  ...
  const nextDayPrediction = useMemo(() => {
    ...
    // Calculate suggested entry price for next day using last trade's close price
    const currentOperation = params.operation || 'buy';
    const currentEntryPercentage = Number(appliedEntryPercentage) || 0;
    ...
    // Calculate stop price for next day
    const currentStopPercentage = Number(appliedStopPercentage) || 0;
    const stopPercent = currentStopPercentage / 100;
    let stopPrice = 0;
  ...
  return (
    <div className="w-full flex flex-col gap-4 md:gap-6">
      ...
      <div className={`${isMobile ? 'space-y-4' : 'grid md:grid-cols-4 gap-4'}`}>
        ...
        <div ref={setupPanelRef} className={`${isMobile ? 'order-2' : 'md:col-span-1 h-[460px]'} bg-card rounded-lg border p-3 md:p-4 flex flex-col`}>
      ...
      <div className="md:hidden space-y-3 mb-6">
        ...
        {isLoading ? (
          ...
          <>
            ...
            {/* Next Day Prediction Card for Mobile */}
            {nextDayPrediction && (
              <div className="bg-blue-50 dark:bg-blue-950/20 border-2 border-blue-200 dark:border-blue-800 rounded-lg p-3 space-y-2">
                <div className="flex justify-between items-center pb-2 border-b border-blue-200 dark:border-blue-800">
                  <span className="font-medium text-sm text-blue-700 dark:text-blue-300">
                    {formatDate(nextDayPrediction.date)} (Next Day)
                  </span>
                ...
                <div className="grid grid-cols-2 gap-2 text-xs">
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium text-blue-700 dark:text-blue-300">
                        {formatPrice(nextDayPrediction.stopPrice)}
                      </span>
                    </div>
                  </div>
                </div>
            ...
            <TableBody>
              {isLoading ? (
                ...
                <>
                  ...
                  {/* Next Day Prediction Row for Desktop */}
                  {nextDayPrediction && (
                    <TableRow className="bg-blue-50 dark:bg-blue-950/20 hover:bg-blue-100 dark:hover:bg-blue-950/30 border-t-2 border-blue-200 dark:border-blue-800">
                      {columns.map((column) => {
                        let cellContent = "-";
                        ...
                        if (column.id === "date") {
                          ...
                          cellClass += " font-medium text-blue-700 dark:text-blue-300";
                        } else if (column.id === "suggestedEntryPrice") {
                          cellContent = formatPrice(nextDayPrediction.suggestedEntryPrice);
                          cellClass += " font-medium text-blue-700 dark:text-blue-300";
