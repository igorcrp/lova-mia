...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const login = async (email: string, password: string) => {
    try {
      ...
      if (response.error || !response.session) {
        throw new Error(response.error?.message || "Invalid login response from API");
      }
  ...
  const googleLogin = async () => {
    try {
      ...
      if (response.error || !response.user?.email) {
        throw new Error(response.error?.message || 'Failed to get user email from Google login');
      }
