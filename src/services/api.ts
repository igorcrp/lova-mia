...
const analysisService = {
      ...
      for (let i = 0; i < stocksToProcess.length; i++) {
        ...
        try {
          ...
          results.push({
            ...
            finalCapital: lastCurrentCapital, // Use the last Current Capital as Final Capital
  ...
  calculateDetailedMetrics(stockData: any[], tradeHistory: TradeHistoryItem[], capitalEvolution: any[], params: StockAnalysisParams, stockCode?: string) {
    ...
    // Final Capital = Current Capital da data mais recente (garantir ordenação por data)
    ...
    if (tradeHistory.length > 0) {
      ...
      finalCapital = lastTrade.currentCapital || params.initialCapital;
      
      // Debug log para verificar o valor
      console.log(`[${stockCode || 'unknown'}] Final Capital Debug:`, {
  ...
  async getDetailedAnalysis(
    ...
    try {
      ...
      return {
        ...
        capitalEvolution,
        initialBalance: params.initialCapital,
        finalBalance: metrics.finalCapital,
