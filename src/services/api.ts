      ...
      if (authData.user) {
        ...
        try {
          ...
          if (stripeError) {
            ...
            // Don't throw error - registration should still succeed even if Stripe customer creation fails
          } else {
...
const analysisService = {
  ...
  async runAnalysis(
    ...
    try {
      ...
      for (let i = 0; i < stocksToProcess.length; i++) {
        ...
        console.info(`DEBUG: Processing stock ${i+1}/${stocksToProcess.length}: ${stock.code}`);
        ...
        try {
          ...
          console.info(`DEBUG: ${stock.code} - Retrieved ${stockData?.length || 0} data points`);
          
          if (!stockData || stockData.length === 0) {
            console.warn(`DEBUG: No data found for stock ${stock.code}, skipping`);
            // Still add a placeholder result for debugging
            results.push({
          ...
          console.info(`DEBUG: ${stock.code} - Generated ${tradeHistory?.length || 0} trade history entries`);
          
          if (!tradeHistory || tradeHistory.length === 0) {
            console.warn(`DEBUG: No trade history generated for ${stock.code}, creating minimal result`);
            // Create a minimal result even without trades
            results.push({
              assetCode: stock.code,
              assetName: stock.name || stock.code,
          ...
          // Get the most recent Current Capital from trade history (garantir ordenação por data)
          let lastCurrentCapital = params.initialCapital;
          if (tradeHistory.length > 0) {
            // Ordenar por data para garantir que pegamos o valor da data mais recente
      ...
      // Debug log para verificar se a correção está funcionando
      let lastTradeDate = 'N/A';
      if (tradeHistory.length > 0) {
        const sortedTradeHistory = [...tradeHistory].sort((a, b) => 
          new Date(a.date).getTime() - new Date(b.date).getTime()
        );
      ...
      console.log(`[${stock.code}] CORRECTED Final Capital:`, {
        totalTrades: tradeHistory.length,
        lastTradeDate: lastTradeDate,
        lastCurrentCapital: lastCurrentCapital,
        finalCapitalFromMetrics: 'REMOVED to avoid override'
      });
  ...
  async updateDetailedAnalysisOptimized(
    ...
    try {
      console.info(`[updateDetailedAnalysisOptimized] CRITICAL FIX: Using EXACT same logic as premiumAnalysisService`);
      
      // Ordenar trade history por data para processamento sequencial
      const sortedTradeHistory = [...existingResult.tradeHistory].sort((a, b) => 
        new Date(a.date).getTime() - new Date(b.date).getTime()
      );
      
      let capital = params.initialCapital;
      
      // USAR A MESMA LÓGICA DO premiumAnalysisService.ts
      const updatedTradeHistory = sortedTradeHistory.map((currentData, i) => {
        const previousData = i > 0 ? sortedTradeHistory[i - 1] : null;
        const previousCapital = i > 0 ? capital : params.initialCapital;
        
        // USAR PRICE DO DIA ANTERIOR (como no premiumAnalysisService)
        const referencePrice = previousData ? this.getReferencePrice(previousData, params.referencePrice) : this.getReferencePrice(currentData, params.referencePrice);
        
        // Calcular usando a MESMA LÓGICA do premiumAnalysisService
        const suggestedEntryPrice = params.operation === 'buy'
          ? referencePrice - (referencePrice * params.entryPercentage / 100)
          : referencePrice + (referencePrice * params.entryPercentage / 100);
        
        const openPrice = Number(currentData.entryPrice);
        const lowPrice = Number(currentData.low);
        const highPrice = Number(currentData.high);
        const closePrice = Number(currentData.exitPrice);
        
        // CALCULAR ACTUAL PRICE usando MESMA LÓGICA
        let actualPrice: number | string;
        if (params.operation === 'buy') {
        ...
        // CALCULAR LOT SIZE usando MESMA LÓGICA
        const lotSize = actualPrice !== '-' && previousCapital > 0 && Number(actualPrice) > 0
          ? Math.floor(previousCapital / Number(actualPrice) / 10) * 10 
          : 0;
        
        // CALCULAR TRADE usando MESMA LÓGICA
        let trade: string;
        if (params.operation === 'buy') {
        ...
        // CALCULAR STOP PRICE usando MESMA LÓGICA
        const stopPrice = actualPrice !== '-' ? (params.operation === 'buy'
          ? Number(actualPrice) - (Number(actualPrice) * params.stopPercentage / 100)
          : Number(actualPrice) + (Number(actualPrice) * params.stopPercentage / 100)) : '-';
        
        // CALCULAR STOP TRIGGER usando MESMA LÓGICA
        let stopTrigger: string = '-';
        if (trade !== "-" && stopPrice !== '-') {
          if (params.operation === 'buy') {
        ...
        // CALCULAR PROFIT/LOSS usando MESMA LÓGICA
        let profitLoss = 0;
        if (trade !== "-" && actualPrice !== '-') {
          if (stopTrigger === "Executed" && stopPrice !== '-') {
            profitLoss = params.operation === 'buy'
        ...
        // CALCULAR CURRENT CAPITAL usando MESMA LÓGICA
        capital = Math.max(0, previousCapital + profitLoss);
        ...
        return {
          ...
          trade,
          lotSize,
          stopPrice: typeof stopPrice === 'number' ? Number(stopPrice.toFixed(2)) : stopPrice,
          stopTrigger,
