...
const stockData = {
  async getAvailableStocks(tableName: string): Promise<StockInfo[]> {
    try {
      ...
      console.error(`Failed to get stocks for ${tableName}:`, error);
      return [];
    }
  },

  async getStockData(tableName: string, stockCode: string, period?: string): Promise<any[]> {
    try {
      if (!tableName || !stockCode) throw new Error('Table name and stock code are required');
      
      let query = fromDynamic(tableName)
        .select('*')
